...
def get_roms_data(path: Path, opts: Options) -> dict[str, dict]:
    if opts.logs: log(f"[DATA COLLECTOR]: Fetching and organizing data from {path}")
    ...
    for console in path.glob("*"):
        if not console.is_dir(): 
            if opts.logs: log(f"[DATA COLLECTOR]: Ignoring non-directory item {console.name}")
            continue

        if opts.logs: log(f"[DATA COLLECTOR]: Scanning console {console.name}")
        ...
        for sub in console.glob("*"):
            if sub.is_dir() and is_valid_subfolder(sub.name, opts.valid_subfolders):
                for game in sub.glob("*"):

                    # Inside single or multidisk
                    games_per_console[console.name] += 1
                    add_game_data(games_data, game, console, opts.logs, identifier)
                    identifier += 1
                ...
                games_per_console[console.name] += 1
                add_game_data(games_data, sub, console, opts.logs, identifier)
                identifier += 1
    ...
    if opts.logs: log(f"[DATA COLLECTOR]: Collection of data finalized - {len(bytes_per_console)} consoles processed.")
